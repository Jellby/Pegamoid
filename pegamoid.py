...
class Orbitals(object):
  ...
  def create_inporb(self, filename):
    ...
    with open(filename, 'w') as f:
      ...
      f.write(wrap_list([int(uhf), len(self.N_bas), 0], 3, '{:7d}')[0])
      f.write('\n')
      f.write(wrap_list(self.N_bas, 8, '{:7d}')[0])
      f.write('\n')
      f.write(wrap_list(self.N_bas, 8, '{:7d}')[0])
      f.write('\n')
      ...
      for s,(i,j) in enumerate(nMO):
        for k in range(i,j):
          ...
          cff = wrap_list(self.MO[k]['coeff'][i:j], 5, '{:21.14E}')
          f.write(' ' + '\n '.join(cff) + '\n')
      if (uhf):
        f.write('#UORB\n')
        for s,(i,j) in enumerate(nMO):
          for k in range(i,j):
            f.write('* ORBITAL{0:5d}{1:5d}\n'.format(s+1, k-i+1))
            cff = wrap_list(self.MO_b[k]['coeff'][i:j], 5, '{:21.14E}')
            f.write(' ' + '\n '.join(cff) + '\n')
      f.write('#OCC\n')
      f.write('* OCCUPATION NUMBERS\n')
      for i,j in nMO:
        occ = wrap_list([o['occup'] for o in self.MO[i:j]], 10, '{:11.4E}')
        f.write(' ' + '\n '.join(occ) + '\n')
      if (uhf):
        f.write('#UOCC\n')
        f.write('* Beta OCCUPATION NUMBERS\n')
        for i,j in nMO:
          occ = wrap_list([o['occup'] for o in self.MO_b[i:j]], 10, '{:11.4E}')
          f.write(' ' + '\n '.join(occ) + '\n')
      f.write('#ONE\n')
      f.write('* ONE ELECTRON ENERGIES\n')
      for i,j in nMO:
        ene = wrap_list([o['ene'] for o in self.MO[i:j]], 10, '{:11.4E}')
        f.write(' ' + '\n '.join(ene) + '\n')
      if (uhf):
        f.write('#UONE\n')
        f.write('* Beta ONE ELECTRON ENERGIES\n')
        for i,j in nMO:
          ene = wrap_list([o['ene'] for o in self.MO_b[i:j]], 10, '{:11.4E}')
          f.write(' ' + '\n '.join(ene) + '\n')
      f.write('#INDEX\n')
      f.write('\n'.join(index))
      f.write('\n')
...
def create_index(MO, MO_b, nMO):
  ...
  for s in nMO:
    ...
    for j,l in enumerate(wrap_list(types, 10, '{}')):
      index.append('{0} {1}'.format(j, l.replace(' ', '')))
  return index
...
class Viewer(HasTraits):
  ...
  def write_cube(self, *args):
    ...
    if (dlg.open() == OK):
      ...
      grid = [i/(j-1) for i,j in zip(self.edge, ngrid)]
      with open(dlg.path, 'w') as f:
        f.write('File generated by {0} from {1}\n'.format(__name__, self.file))
        f.write('{0}\n'.format(self.orblist[self.orb].split(':', 1)[1]))
        f.write('{0:5d} {1:11.6f} {2:11.6f} {3:11.6f}\n'.format(len(self.orbitals.centers), np.amin(self.xyz[0]), np.amin(self.xyz[1]), np.amin(self.xyz[2])))
        f.write('{0:5d} {1:11.6f} {2:11.6f} {3:11.6f}\n'.format(ngrid[0], grid[0], 0, 0))
        f.write('{0:5d} {1:11.6f} {2:11.6f} {3:11.6f}\n'.format(ngrid[1], 0, grid[1], 0))
        f.write('{0:5d} {1:11.6f} {2:11.6f} {3:11.6f}\n'.format(ngrid[2], 0, 0, grid[2]))
        for c in self.orbitals.centers:
          f.write('{0:5d} {1:11.6f} {2:11.6f} {3:11.6f} {4:11.6f}\n'.format(c['Z'], 0, *c['xyz']))
        for x in self.vol:
          for y in x:
            f.write('\n'.join(wrap_list(y, 6, '{:13.4E}')))
            f.write('\n')
